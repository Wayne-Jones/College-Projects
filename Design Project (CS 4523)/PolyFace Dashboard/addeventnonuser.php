<?php 
	...
	if ((($_FILES["file"]["type"] == "image/gif") || ($_FILES["file"]["type"] == "image/jpeg") || ($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/pjpeg") || ($_FILES["file"]["type"] == "image/x-png") || ($_FILES["file"]["type"] == "image/png")) && ($_FILES["file"]["size"] < 400000) && in_array($extension, $allowedExts))
	{
		...
		//Execute the query
		$query= "insert into event(NAMEOFEVENT,TYPEOFEVENT,DATEOFEVENT,TIMEOFEVENT,LOCATIONOFEVENT,IMGPATH,ISPUBLISHED,EVENTDESCRIPTION) values(:nameOfEvent, :typeOfEvent, :dateOfEvent, :timeOfEvent, :locationOfEvent, :imgPath, '0', :eventDesc)";
		$name = $_POST["name"];
		$type = $_POST["type"];
		$date = $_POST["date"];
		$time = $_POST["time"];
		...
		$eventPending="Your event has been posted for pending";
		$_SESSION["eventPending"]=$eventPending;
		header("refresh:1; index.html");
	}
	else
	{
